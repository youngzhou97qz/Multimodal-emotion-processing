...
log_dir = '/home/'+user+'/par_log/log_fix_gru/'
L_DIM = 300
V_DIM = 35
A_DIM = 74
L_LEN = 30
...
class Transformer_Blocks(nn.Module):
    def __init__(self, dim, n_heads, n_layers, ffn):
        super().__init__()
        self.normalization = nn.ModuleList([nn.LayerNorm(dim) for _ in range(n_layers*4)])
...
class Multi_class(nn.Module):
    def __init__(self, l_dim, v_dim, a_dim, dim, l_len, v_len, a_len, n_heads, n_layers, ffn):
        ...
        self.multimodal_blocks = nn.ModuleList([Transformer_Blocks(dim, n_heads, n_layers, ffn) for _ in range(9)])
        self.transformer_blocks = nn.ModuleList([Transformer_Blocks(dim*3, n_heads, n_layers, ffn) for _ in range(3)])
...
class Our_GRU(nn.Module):
    def __init__(self, input_dim, output_dim, bias=True):
        ...
        self.x2h = nn.Linear(input_dim, output_dim*3, bias=bias)
        self.h2h = nn.Linear(output_dim, output_dim*3, bias=bias)
        self.trans = nn.Linear(output_dim, output_dim, bias=False)
    def forward(self, x):
        output = []
        hidden = torch.zeros([x.shape[0], self.output_dim], dtype=torch.float, device=device, requires_grad=True)
        for seq in range(x.shape[1]):
            x2h = self.x2h(x[:,seq,:])
            h2h = self.h2h(hidden)
            x_u, x_r, x_t = x2h.chunk(3, 1)
            h_u, h_r, h_t = h2h.chunk(3, 1)
            update_gate = torch.sigmoid(h_u + x_u)
            reset_gate = torch.sigmoid(h_r + x_r)
            transfer_h = torch.tanh(self.trans(reset_gate * x_t))
            hidden = (1 - update_gate) * hidden + update_gate * transfer_h
...
class State_Transfer(nn.Module):
    def __init__(self, l_dim, v_dim, a_dim, dim, l_len, v_len, a_len, n_heads, n_layers, ffn):
        ...
        self.transfer = Our_GRU(input_dim=dim, output_dim=6)
    def forward(self, l, v, a, l_mask, v_mask, a_mask):  # (batch, y_len, x_len, x_dim)
        ...
        for i in range(l.shape[1]):
            ...
            temp_feats = self.feature(temp_l, temp_v, temp_a, temp_l_mask, temp_v_mask, temp_a_mask)
            feats_list.append(temp_feats.unsqueeze(1))
        feats = torch.cat(feats_list, dim=1)
        feats = self.transfer(feats)
        return feats  # (batch, y_len, 6)
...
# log_fix_gru
model_1 = State_Transfer(l_dim=L_DIM, v_dim=V_DIM, a_dim=A_DIM, dim=DIM, l_len=L_LEN, v_len=V_LEN, a_len=A_LEN, n_heads=N_HEADS, n_layers=N_LAYERS, ffn=FFN).to(device)
model_1.load_state_dict(torch.load(log_dir + 'model_2_1.52.pt'))
model_2 = State_Transfer(l_dim=L_DIM, v_dim=V_DIM, a_dim=A_DIM, dim=DIM, l_len=L_LEN, v_len=V_LEN, a_len=A_LEN, n_heads=N_HEADS, n_layers=N_LAYERS, ffn=FFN).to(device)
model_2.load_state_dict(torch.load(log_dir + 'model_1_1.62.pt'))
